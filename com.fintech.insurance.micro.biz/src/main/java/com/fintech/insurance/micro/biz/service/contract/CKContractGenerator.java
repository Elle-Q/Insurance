...
/**
 * @Description: 车险分期合同产生器
 * @Author: Yong Li
 * @Date: 2018/1/6 13:33
 */
@Service("ckContractGenerator")
public class CKContractGenerator extends AbstractContractGenerator {
    ...
    public ContractInfoResponseVO buildServiceContract(Integer contractId) {
        ...
        return generateContractFileData(isSign, valueMap,
                "车贷分期服务合同模板.ftl", 4,  customerVO, true);
    }
    ...
    public ContractInfoResponseVO buildBorrowContract(Integer contractId) {
        ...
        valueMap.put("insuranceAccountName", requisition.getLoanAccountName());
        valueMap.put("insuranceCompanyName", requisition.getInsuranceCompanyName());
        valueMap.put("insuranceBankName", String.format("%s%s", requisition.getLoanAccountBank(), requisition.getLoanAccountBankBranch()));
        valueMap.put("insuranceBankcard", requisition.getLoanAccountNumber());

        Product product = contract.getRequisition().getProduct();
        ProductRate monthRate = productRateDao.getProductRateByProductIdAndDuration(product.getId(), 12);
        ...
        valueMap.put("debtBankcardUserName", "-");
        valueMap.put("debtBankcardNum", "-");
        valueMap.put("debtBankcardName", "-");

        if (StringUtils.isNotBlank(requisition.getPaymentOrderNumber())) {//服务费支付单已经生成
            FintechResponse<PaymentOrderVO> paymentOrderVOFintechResp = paymentOrderServiceFeign.getByOrderNumber(requisition.getPaymentOrderNumber());
            ...
            if (null != paymentOrder && StringUtils.isNotBlank(paymentOrder.getPaymentAccountNumer())) {
                ...
                if (bankcardResp.isOk() && bankcardResp.getData() != null) {
                    ...
                    valueMap.put("debtBankcardUserName", customerVO.getName());
                    valueMap.put("debtBankcardNum", bankCardVO.getAccountNumber());
                    // 根据银行代码获取银行名称
                    ...
                    if (bankResp.isOk() && null != bankResp.getData()) {
                        valueMap.put("debtBankcardName", bankResp.getData().getBankName());
                    }
                }
            }
        }
        ...
        boolean isSign = StringUtils.isNoneBlank(contract.getCustomerContractNumber());
        return generateContractFileData(isSign, valueMap,
                "车贷分期借款合同模板.ftl", 8,  customerVO, false);
        ...
        valueMap.put("insuranceFee", "100.23");

        valueMap.put("borrowAmount", "100.23");
        valueMap.put("borrowAmountCap", "test");

        valueMap.put("insuranceAccountName", "测试保险公司");


        valueMap.put("insuranceBankName", "test");
        valueMap.put("insuranceBankcard", "test");
        valueMap.put("insuranceCompanyName","abc");
        ...
        valueMap.put("securityAmount", "1000");
        valueMap.put("securityAmountCap", "一二三");

        valueMap.put("repayMethod", "AAAAAAAA");

        valueMap.put("overdueFeeRate", "1.23");
        ...
        valueMap.put("icon", ImageUtils.getImageContentString(this.getClass().getClassLoader().getResourceAsStream("nuomi_icon.png")));

        CustomerVO customerVO = new CustomerVO();
        customerVO.setMobile("13902443572");

        try {
            WordUtil.createWordFile(valueMap, "车贷分期借款合同模板.ftl", new File("serviceContract.doc"), ContractService.class);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    protected ContractSignVO buildBorrowContractSignInfo(String userAccountId, String bestsignFileId) {
        ...
        signVO.setTitle("车贷分期借款合同");
        signVO.setDescription("车贷分期借款合同");
        signVO.setContractSignUserType(ContractSignUserType.LENDER);

        signVO.getUserSignLocations().add(new SignLocationVO(7, 0.64d, 0.346d));
        signVO.getEnterpriseSignLocations().add(new SignLocationVO(7, 0.64d, 0.21d));
    ...
    protected ContractSignVO buildServiceContractSignInfo(String userAccountId, String bestsignFileId) {
        ...
        signVO.setUserAccountId(userAccountId);
        signVO.setContractFileNum(bestsignFileId);
        signVO.setTitle("车贷分期服务合同");
        signVO.setDescription("车贷分期服务合同");

        //"pageNum":4, "xaxis": 0.25, "yaxis": "0.787"
        signVO.getUserSignLocations().add(new SignLocationVO(5, 0.25d, 0.21d));
        //"pageNum":4, "xaxis": 0.6, "yaxis": "0.79"}
        signVO.getEnterpriseSignLocations().add(new SignLocationVO(5, 0.6d, 0.21d));
